...
/**
 * @file Woodcutter recipe additions for Valhelsia 3.
 * 
 * Includes functions for frequently used recipe templates.
 * 
 * Note: This script is incredibly ugly and needs far more cleanup work done.
 * I just don't have the motivation to clean it up now due to the shear amount
 * of refactoring that is needed.
 */
...
onEvent('recipes', event => {
   ...
   * @param {(string|Item)} output The resulting output item.
   * @param {(string|Ingredient)} inputs The input, usually a log, wood, or plank.
   * @param {number} count The number of items to output.
   */
  ...
  // TODO: Move these wood constants into constants.js and rename.
  ...
  // Structurize - Wooden Timber Frames
  for (pattern of STRUCTURIZE_PATTERNS) {
    for (wood of STRUCTURIZE_WOODS) {
      for (material of STRUCTURIZE_WOODEN_MATERIALS) {
        for (altPattern of STRUCTURIZE_PATTERNS) {
          if (pattern === altPattern) {
            continue;
          }
          woodcutting(`structurize:${altPattern}_${wood}_${material}_timber_frame`, `structurize:${pattern}_${wood}_${material}_timber_frame`, 1);
        }
      }
    }
  }
